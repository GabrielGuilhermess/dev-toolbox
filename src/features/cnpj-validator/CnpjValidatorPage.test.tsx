...
async function renderValidatorPage(): Promise<ReactElement> {
  const { default: CnpjValidatorPage } = await import('./index');

  return createElement(CnpjValidatorPage);
}

describe('CnpjValidatorPage', () => {
  ...
  it('carrega a pagina na rota /cnpj-validator via lazy loading', async () => {
    window.history.pushState({}, '', '/cnpj-validator');
    ...
    expect(
      await screen.findByRole('heading', { name: 'Validador de CNPJ' }, { timeout: 5000 }),
    ).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /validar/i })).toBeInTheDocument();

    await waitFor(() => {
      expect(document.title).toBe('Validador de CNPJ | Dev Toolbox');
    });
  }, 10000);

  it('valida um CNPJ, copia o resultado, mostra status invalido e limpa a tela', async () => {
    render(createElement(ToastProvider, undefined, await renderValidatorPage()));

    const input = screen.getByLabelText<HTMLTextAreaElement>('CNPJ');
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('CNPJ valido.')).toBeInTheDocument();
    });
    ...
    fireEvent.change(input, {
      target: { value: '11.222.333/0001-00' },
    });
    ...
    await waitFor(() => {
      expect(output.value).toContain('CNPJ formatado: 11.222.333/0001-00');
      expect(output.value).toContain('Status: CNPJ invalido');
      expect(screen.getByText('CNPJ invalido.')).toBeInTheDocument();
    });
  ...
  it('exibe toast de erro quando o CNPJ nao pode ser validado', async () => {
    render(createElement(ToastProvider, undefined, await renderValidatorPage()));

    fireEvent.click(screen.getByRole('button', { name: /validar/i }));

    expect(await screen.findByText('Informe um CNPJ.')).toBeInTheDocument();
